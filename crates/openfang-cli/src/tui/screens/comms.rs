...
pub fn draw(f: &mut Frame, area: Rect, state: &mut CommsState) {
    ...
    let hint_text = if !state.status_msg.is_empty() {
        format!(
            "  {} | [s]发送  [t]任务  [r]刷新  [Tab]焦点  [\u{2191}\u{2193}]滚动",
            state.status_msg
        )
    } else {
        "  [s]发送  [t]任务  [r]刷新  [Tab]焦点  [\u{2191}\u{2193}]滚动".to_string()
    };
    f.render_widget(
...
fn draw_topology(f: &mut Frame, area: Rect, state: &CommsState) {
    ...
    if state.nodes.is_empty() {
        f.render_widget(
            Paragraph::new(Span::styled("  没有代理在运行。", theme::dim_style())),
            area,
        );
        return;
    }
...
fn draw_send_modal(f: &mut Frame, area: Rect, state: &CommsState) {
    ...
    f.render_widget(
        Paragraph::new(Span::styled("来自 (代理ID):", field_style(0))),
        rows[0],
    );
    f.render_widget(
        Paragraph::new(Span::styled(
    ...
    f.render_widget(
        Paragraph::new(Span::styled("至 (代理ID):", field_style(1))),
        rows[2],
    );
    f.render_widget(
        Paragraph::new(Span::styled(
