...
fn draw_list(f: &mut Frame, area: Rect, state: &mut ChannelState) {
    ...
    if state.loading {
        ...
        ChannelStatus::MissingEnv => ("[缺少环境变量]", theme::channel_missing()),
    ...
    let hints = Paragraph::new(Line::from(vec![Span::styled(
        "  [\u{2191}\u{2193}] 导航  [Tab] 类别  [回车] 设置  [t] 测试  [e/d] 启用/禁用  [r] 刷新",
        theme::hint_style(),
    )]));
    ...
    match &state.test_result {
        None => {
            ...
            f.render_widget(
                Paragraph::new(Line::from(vec![
                    Span::styled(format!("  {spinner} "), Style::default().fg(theme::CYAN)),
                    Span::styled("检查凭证\u{2026}", theme::dim_style()),
                ])),
                chunks[1],
            );
        }
