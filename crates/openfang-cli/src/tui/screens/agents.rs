...
const TOOL_OPTIONS: &[(&str, &str)] = &[
    ...
    ("file_write", "写入文件"),
    ("file_list", "列出目录内容"),
    ("memory_store", "在代理内存中存储数据"),
    ("memory_recall", "从内存中召回数据"),
    ("web_fetch", "获取网页"),
...
pub fn draw(f: &mut Frame, area: Rect, state: &mut AgentSelectState) {
    ...
    let sub_title = match state.sub {
        ...
        AgentSubScreen::CreateMethod => "创建代理",
        AgentSubScreen::TemplatePicker => "模板",
        AgentSubScreen::CustomName => "自定义 — 名称",
        AgentSubScreen::CustomDesc => "自定义 — 描述",
        AgentSubScreen::CustomPrompt => "自定义 — 系统提示",
        AgentSubScreen::CustomTools => "自定义 — 工具",
        AgentSubScreen::CustomSkills => "自定义 — 技能",
        AgentSubScreen::CustomMcpServers => "自定义 — MCP 服务器",
        AgentSubScreen::Spawning => "正在创建...",
    };
    ...
    match state.sub {
        ...
        AgentSubScreen::CustomName => {
            draw_text_input(f, inner, "代理名称:", &state.custom_name, "my-agent")
        }
...
fn draw_create_method(f: &mut Frame, area: Rect, state: &mut AgentSelectState) {
    ...
    let prompt = Paragraph::new("  你想如何创建你的代理？");
    f.render_widget(prompt, chunks[0]);

    let items = vec![
        ListItem::new(Line::from(vec![
            Span::raw("  从模板中选择"),
            Span::styled("  (预构建代理)", theme::dim_style()),
        ])),
        ListItem::new(Line::from(vec![
            Span::raw("  构建自定义代理"),
            Span::styled("  (选择名称、工具、提示)", theme::dim_style()),
        ])),
    ];
