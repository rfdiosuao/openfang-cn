...
pub fn draw(f: &mut Frame, area: Rect, state: &mut SettingsState) {
    ...
    let hint_text = match state.sub {
        SettingsSub::Providers if state.input_mode => "  [回车] 保存  [Esc] 取消",
    ...
    if state.loading && state.providers.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled("正在加载提供商\u{2026}", theme::dim_style()),
            ])),
            chunks[1],
        );
        ...
        f.render_widget(
            Paragraph::new(vec![
                Line::from(vec![Span::styled(
                    format!("  输入 {provider_name} 的 API 密钥: "),
...
fn draw_models(f: &mut Frame, area: Rect, state: &mut SettingsState) {
    ...
    if state.loading && state.models.is_empty() {
        ...
        f.render_widget(
            Paragraph::new(Span::styled("  没有可用的模型。", theme::dim_style())),
            chunks[1],
        );
    } else {
        let items: Vec<ListItem> = state
...
fn draw_tools(f: &mut Frame, area: Rect, state: &mut SettingsState) {
    ...
    if state.tools.is_empty() {
        f.render_widget(
            Paragraph::new(Span::styled("  没有可用的工具。", theme::dim_style())),
            chunks[1],
        );
    } else {
        let items: Vec<ListItem> = state
