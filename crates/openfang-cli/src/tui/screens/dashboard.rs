...
pub fn draw(f: &mut Frame, area: Rect, state: &mut DashboardState) {
    ...
    let hints = Paragraph::new(Line::from(vec![Span::styled(
        "  [r] 刷新  [a] 转到代理  [\u{2191}\u{2193}] 滚动审计",
        theme::hint_style(),
    )]));
...
fn draw_stat_cards(f: &mut Frame, area: Rect, state: &DashboardState) {
    ...
    let agents_block = Block::default()
        .title(Span::styled(" 代理 ", Style::default().fg(theme::CYAN)))
        .borders(Borders::ALL)
    ...
    let provider_block = Block::default()
        .title(Span::styled(" 提供商 ", Style::default().fg(theme::CYAN)))
        .borders(Borders::ALL)
        .border_style(Style::default().fg(theme::DIM));
    ...
    if state.loading {
        ...
        f.render_widget(
            Paragraph::new(Line::from(vec![
                ...
                Span::styled("正在加载审计轨迹\u{2026}", theme::dim_style()),
            ])),
            area,
        );
        return;
