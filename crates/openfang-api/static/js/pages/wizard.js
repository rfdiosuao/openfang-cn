  ...
  return {
    ...
    async saveKey() {
      ...
      try {
        ...
        OpenFangToast.success('API密钥已保存至 ' + provider.display_name);
        await this.loadProviders();
    ...
    async detectClaudeCode() {
      ...
      try {
        ...
        if (result.status === 'ok') {
          ...
          OpenFangToast.success('Claude Code 已检测到 (' + (result.latency_ms || '?') + 'ms)');
        } else {
          this.testResult = { status: 'error', error: 'Claude Code CLI not detected' };
          OpenFangToast.error('未检测到 Claude Code CLI。请确保您已运行: npm install -g @anthropic-ai/claude-code && claude auth');
        }
      } catch(e) {
        this.testResult = { status: 'error', error: e.message };
    ...
    async createAgent() {
      ...
      try {
        ...
        if (res.agent_id) {
          ...
          OpenFangToast.success('代理 "' + (res.name || name) + '" 已创建');
          await Alpine.store('app').refreshAgents();
        } else {
          OpenFangToast.error('失败: ' + (res.error || '未知错误'));
        }
      } catch(e) {
        OpenFangToast.error('创建代理失败: ' + e.message);
      }
