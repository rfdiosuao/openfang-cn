  ...
  return {
    ...
    async toggleJob(job) {
      try {
        ...
        OpenFangToast.success('计划 ' + (newState ? '已启用' : '已暂停'));
      } catch(e) {
        OpenFangToast.error('切换计划状态失败: ' + (e.message || e));
      }
    },
    ...
    deleteJob(job) {
      ...
      OpenFangToast.confirm('删除计划', '删除 "' + jobName + '"? 此操作不可撤销。', async function() {
        try {
    ...
    async runNow(job) {
      ...
      try {
        ...
        if (result.status === 'completed') {
          OpenFangToast.success('计划 "' + (job.name || '任务') + '" 执行成功');
          job.last_run = new Date().toISOString();
        } else {
          OpenFangToast.error('计划执行失败: ' + (result.error || '未知错误'));
        }
      } catch(e) {
        OpenFangToast.error('立即运行功能暂不适用于cron任务');
      }
      this.runningJobId = '';
    },
    ...
    deleteTrigger(trigger) {
      var self = this;
      OpenFangToast.confirm('删除触发器', '删除此触发器? 此操作不可撤销。', async function() {
        try {
