...
function friendlyError(status, serverMsg) {
  if (status === 0 || !status) return '无法连接到守护进程 — openfang 正在运行吗？';
  if (status === 401) return '未授权 — 请检查您的 API 密钥';
  if (status === 403) return '权限被拒绝';
  if (status === 404) return serverMsg || '资源未找到';
  if (status === 429) return '速率限制 — 请减速后重试';
  if (status === 413) return '请求过大';
  if (status === 500) return '服务器错误 — 请检查守护进程日志';
  if (status === 502 || status === 503) return '守护进程不可用 — 它正在运行吗？';
  return serverMsg || '意外错误 (' + status + ')';
}
  ...
  function request(method, path, body) {
    ...
    return fetch(BASE + path, opts).then(function(r) {
      ...
      if (e.name === 'TypeError' && e.message.includes('Failed to fetch')) {
        ...
        throw new Error('无法连接到守护进程 —— openfang 是否正在运行？');
      }
      throw e;
    });
  }
